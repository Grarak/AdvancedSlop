...
fn main() {
    ...
    if is_target_vita() {
        ...
        println!("cargo:rerun-if-env-changed=ADVANCEDSLOP_SYSROOT");
        if vitabuild::is_target_arm32() {
            // The armhf cross sysroot; a native armhf host links against its own.
            if let Ok(sysroot) = env::var("ADVANCEDSLOP_SYSROOT") {
    ...
    // Linux builds imgui + the SDL GL3 backend from the vendored sources; the Vita links
    // the vitasdk's prebuilt imgui instead.
...

    {
        ...
        let kubridge_lib_new_path = kubridge_out.join("libkubridge_stub_advancedslop.a");
        ...
        println!("cargo:rustc-link-lib=static=kubridge_stub_advancedslop");
    }
