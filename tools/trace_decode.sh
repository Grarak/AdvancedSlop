...
#
# The decoder is a small detached crate (tools/trace-decode) that reuses advancedslop's REAL
# disassembler via #[path] but links no C/C++ (see its Cargo.toml), so it runs natively on the
# x86 dev box — unlike the in-emulator decoder, which only exists in the arm build.
