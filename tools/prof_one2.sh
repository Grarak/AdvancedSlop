...
# Corrected unified profiler: touch build + ADVANCEDSLOP_DBG_TOUCH, longer looping drive-in that
...
BIN="${ADVANCEDSLOP_PI_BIN/#\~/$HOME}"; NAME=$(basename "$BIN")
D="$HOME/prof"
mkdir -p "$D"
export DISPLAY="$ADVANCEDSLOP_DISPLAY" LIBGL_ALWAYS_SOFTWARE=1 ADVANCEDSLOP_DBG_TOUCH=1
export XDG_RUNTIME_DIR="$ADVANCEDSLOP_PI_RUNTIME_DIR" WAYLAND_DISPLAY="$ADVANCEDSLOP_PI_WAYLAND_DISPLAY"
WT="$ADVANCEDSLOP_PI_WTYPE"
