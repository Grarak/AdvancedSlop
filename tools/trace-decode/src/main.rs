// Standalone x86 decoder for advancedslop binary instruction traces (.ilog).
//
// advancedslop's own decoder is compiled into the emulator, which only builds for arm/aarch64 — so a
// trace pulled to the x86 dev box can't be decoded there. This binary shares advancedslop's REAL
...
//   --cpu N    only print inst records for cpu N (GBA records are always cpu 1); text/mem lines are dropped
