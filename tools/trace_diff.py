...
# (as produced by advancedslop --inst-log / --inst-log-lazy with DEBUG_LOG=true).
#
...
# The record format keeps a cpu byte; GBA traces are single-cpu (always 1), so
# the filter only matters for diffing against old dual-cpu-era captures.
CPU_FILTER = None
...
def read_inst(f, prev):
    ...
    while True:
        ...
        if t == TAG_INST:
            ...
            # Single-cpu GBA logs: keyframes stamp cpu=0 while deltas still set FLAG_CPU7;
            # fold everything onto slot 0.
            cpu = 0
            p = prev[cpu]
            if p is None:
