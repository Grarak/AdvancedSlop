...
# (relative to $ADVANCEDSLOP_ROMS_DIR; default $HOME/prof_keep.txt).
...
while IFS= read -r rom; do
  [ -z "$rom" ] && continue
  n=$((n+1))
  out="g$(printf '%03d' "$n")_$(echo "$rom" | sed 's/\.gba$//; s/[^A-Za-z0-9]/_/g' | cut -c1-40)"
  echo "[$n/$TOTAL] $(date '+%H:%M:%S') START $rom -> $out" >> "$MASTER"
  res=$(bash "$(dirname "$0")/prof_one2.sh" "$ADVANCEDSLOP_ROMS_DIR/$rom" "$out" 16 25 2>&1)
