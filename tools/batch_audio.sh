...
# (relative to $ADVANCEDSLOP_ROMS_DIR; default $HOME/prof_worst.txt).
set -u
. "$(dirname "$0")/env.sh"
...
while IFS= read -r rom; do
  [ -z "$rom" ] && continue
  n=$((n+1))
  out="a$(printf '%03d' "$n")_$(echo "$rom" | sed 's/\.gba$//; s/[^A-Za-z0-9]/_/g' | cut -c1-40)"
  echo "[$n/$TOTAL] $(date '+%H:%M:%S') START $rom -> $out" >> "$MASTER"
  res=$(bash "$(dirname "$0")/prof_one2_audio.sh" "$ADVANCEDSLOP_ROMS_DIR/$rom" "$out" 16 25 2>&1)
