...
set -e
. "$(dirname "$0")/env.sh"
require_env ADVANCEDSLOP_SYSROOT
cd "$ADVANCEDSLOP_ROOT"
qemu-arm -L "$ADVANCEDSLOP_SYSROOT" target/thumbv7neon-unknown-linux-gnueabihf/release-debug/advancedslop decode-inst-log "$1" 2>/dev/null \
