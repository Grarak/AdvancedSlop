---
name: profiling
description: Profile AdvancedSlop on the Linux ARM test box — flat perf profiles, per-instruction annotation of native functions and inside JIT blocks (jitdump), and A/B throughput benchmarking. Use when asked to profile, find hotspots, annotate, or measure a performance change.
---

# Profiling AdvancedSlop

Prerequisite: `.env` with `ADVANCEDSLOP_PI_HOST` (see run-on-testbox skill for deploy/keys/rules).
...
The emulator always writes `/tmp/perf-<pid>.map` on Linux (`ARM7_<guest_pc>` symbols).
...
```bash
ADVANCEDSLOP_JITDUMP=1 ./advancedslop <rom> ...           # writes /tmp/jit-<pid>.dump
...
perf annotate -i out.jitted.data --stdio -s ARM7_<guest_pc>
```
...
Top `ARM7_<pc>` symbol → dump live guest memory through the fastmem mirror
