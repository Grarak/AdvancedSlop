...
**Trace hello_world.gba first** (maintainer rule) — it's the cheapest gate: tiny, boots
...
3. For an A/B pair, build the two variants as SEPARATE edit+build commands (chained
   ...
   "Compiling advancedslop" line appears, and `md5sum` both binaries to prove they differ.

## Capture

| mode | how |
|---|---|
| From boot | `advancedslop --inst-log <path> <rom> ...` (boot with NO input = deterministic, diffs cleanly) |
| Final stretch only | `advancedslop --inst-log-lazy <path> ...` with `ADVANCEDSLOP_DBG_PORT=<port>`, then send `inst-log` to the port at the interesting moment (`printf 'inst-log\n' \| nc -q0 127.0.0.1 <port>`) |
| Flush + stop | `kill -INT <pid>` (the panic hook also flushes — a crash self-captures its tail) |

Local capture: `tools/trace.sh <out.ilog>` boots `$ADVANCEDSLOP_TEST_ROM` under qemu.

## Decode & analyze

```bash
tools/trace_decode.sh <path> > out.txt         # exact per-inst text (regs + InstInfo), x86-NATIVE
grep -a "^ARM7 Executed" out.txt               # register-state stream
tools/trace_decode.sh <path> --cpu 1 --index   # cpu filter (GBA records are always cpu 1), number records
...
`trace_decode.sh` builds/runs `tools/trace-decode` — a detached crate that reuses advancedslop's REAL
disassembler (`#[path]` includes of `src/jit/disassembler/**`, `inst_info`, `op`) but links no
C/C++, so it decodes on the x86 dev box where there is no advancedslop binary. Its output is identical
to the in-emulator `advancedslop decode-inst-log <path>` (which still works on an arm box).
...
- **Never decode or analyze traces on the test box** — always scp the .ilog to the dev
  ...
  dev box (no advancedslop binary) decode with `tools/trace_decode.sh` and diff with `trace_diff.py`
  — both run natively there.
- **Only run a built binary locally when the dev machine's arch matches it.** On an **x86
  dev machine** the armhf binary only runs under `qemu-arm` (fine for capture, slow for
  play); for native-speed captures use the pi. Either way pull the ilogs to decode/diff
  locally.
- **A byte-identical boot-window diff is NECESSARY BUT NOT SUFFICIENT** for a jit/emitter
  ...
  code paths, a rarely-hit interrupt). An emitter change once passed a 4M-record boot diff
...
- `ADVANCEDSLOP_INST_LOG_TEXT=0` drops the interleaved text records — on a commercial boot they
...
- The very frequent mem.rs "memory read/write at" lines are compact binary TAG_MEM records
  ...
  `ADVANCEDSLOP_INST_LOG_TEXT=0` drops them like any text. Logs from before July 10 2026 still
  decode; older binaries can't read new logs.
- `ADVANCEDSLOP_INST_LOG_MAX=N` stops the log after exactly N records, flushed from the logging
