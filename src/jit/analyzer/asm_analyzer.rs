...
fn analyze_branch_label(insts: &[InstInfo], thumb: bool, branch_index: usize, cond: Cond, pc: u32, target_pc: u32) -> JitBranchInfo {
    // Backward conditional branch over a side-effect-free body (no stores, no
    // branches/swi/psr ops, no reg read-before-write — is_idle_loop): the block can
    // only leave when an irq handler changes the polled state, so the dispatcher may
    // advance the clock event-by-event instead of spinning (execute_jit's idle path).
    // Measured motivation: Emerald's VBlankIntrWait replacement busy-polls a flag its
    // vblank ISR sets — one 3-instruction poll block owned 14.9% of uncapped cycles.
    // Off in trace builds: skipping loop passes splits interpreter-vs-jit traces
    // structurally.
    if !crate::DEBUG_LOG && (cond as u8) < (Cond::AL as u8) && target_pc < pc {
