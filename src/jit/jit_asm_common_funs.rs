macro_rules! exit_guest_context {
    ($asm:expr) => {{
        ...
        // The pop above loads pc; control never falls through.
...
pub struct JitAsmCommonFuns;

impl JitAsmCommonFuns {
    pub extern "C" fn debug_push_return_stack(current_pc: u32, lr_pc: u32, stack_size: usize) {
        branch_println!("push {lr_pc:x} to return stack with size {stack_size} at {current_pc:x}")
    }

    pub extern "C" fn debug_stack_depth_too_big(size: usize, current_pc: u32) {
        branch_println!("stack depth exceeded {size} at {current_pc:x}")
    }
    ...
    pub extern "C" fn debug_branch_lr(current_pc: u32, target_pc: u32) {
        branch_println!("branch lr from {current_pc:x} to {target_pc:x}")
    }

    pub extern "C" fn debug_branch_lr_failed(current_pc: u32, target_pc: u32, desired_pc: u32) {
        branch_println!("failed to branch lr from {current_pc:x} to {target_pc:x} desired: {desired_pc:x}")
    }

    pub extern "C" fn debug_return_stack_empty(current_pc: u32, target_pc: u32) {
        branch_println!("empty return stack {current_pc:x} to {target_pc:x}")
    }
}
