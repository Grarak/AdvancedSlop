...
#[cfg(target_arch = "arm")]
use crate::core::memory::io_gba::io_gba;
...
use crate::jit::inst_mem_handler::{
    ...
    inst_read_mem_handler_multiple_with_cpsr, inst_read_mem_handler_with_cpsr, inst_write_io_mem_handler, inst_write_io_mem_handler_with_cpsr, inst_write_mem_handler,
    inst_write_mem_handler_multiple, inst_write_mem_handler_multiple_with_cpsr, inst_write_mem_handler_with_cpsr, InstMemMultipleParams,
};
...
#[cfg(target_os = "linux")]
fn block_hash_log(guest_pc: u32, thumb: bool, code: &[u8]) {
    if !crate::IS_DEBUG {
        return;
    }
    use std::io::Write;
    ...
    let writer = WRITER.get_or_init(|| {
        std::env::var("ADVANCEDSLOP_BLOCK_HASH_LOG")
            .ok()
            .map(|path| std::sync::Mutex::new(std::io::BufWriter::new(std::fs::File::create(path).unwrap())))
    });
    if let Some(writer) = writer {
        // ADVANCEDSLOP_BLOCK_DUMP_PC=<hex pc> additionally dumps that block's raw bytes — the
        // debugging companion when the gate reports a mismatch.
        if let Ok(dump_pc) = std::env::var("ADVANCEDSLOP_BLOCK_DUMP_PC") {
        ...
        let _ = writeln!(writer, "{guest_pc:x} {} {} {hash:08x}", thumb as u8, code.len());
        let _ = writer.flush();
    }
}
...
pub static BIOS_UNINTERRUPT_ENTRY_ARM7: JitEntry = JitEntry(hle_bios_uninterrupt as _);
...
create_jit_blocks!(
    [ewram, regions::EWRAM_SIZE],
    [iwram, regions::IWRAM_SIZE],
    // Immutable and large: no live ranges, no write protection; entries only
    [rom, regions::ROM_SIZE]
);

#[derive(Default)]
pub struct JitLiveRanges {
    pub ewram: HeapArrayU8<{ (regions::EWRAM_SIZE / JIT_LIVE_RANGE_PAGE_SIZE / 8) as usize }>,
    pub iwram: HeapArrayU8<{ (regions::IWRAM_SIZE / JIT_LIVE_RANGE_PAGE_SIZE / 8) as usize }>,
}
...
// collapses memory mirrors the same way it does for the jit entries.
#[derive(Default)]
pub struct JitExecCounts {
    pub ewram: HeapArrayU8<{ regions::EWRAM_SIZE as usize / 2 }>,
    pub iwram: HeapArrayU8<{ regions::IWRAM_SIZE as usize / 2 }>,
    pub rom: HeapArrayU8<{ regions::ROM_SIZE as usize / 2 }>,
}
...
struct JitPerfMapRecord {
    ...
    // With ADVANCEDSLOP_JITDUMP set, blocks are additionally written as a perf jitdump (code bytes
...
impl JitPerfMapRecord {
    fn new() -> Self {
        ...
        JitPerfMapRecord {
            ...
            jit_dump: std::env::var("ADVANCEDSLOP_JITDUMP")
                .is_ok()
                .then(|| crate::jit::jit_perf_log::JitPerfLog::new(std::path::PathBuf::from("/tmp"))),
        }
    }

    fn record(&mut self, jit_start: usize, jit_size: usize, guest_pc: u32, thumb: bool) {
        use std::io::Write;
        writeln!(self.perf_map, "{jit_start:x} {jit_size:x} ARM7_{guest_pc:x}").unwrap();
        if let Some(dump) = &mut self.jit_dump {
            let code = unsafe { slice::from_raw_parts((jit_start & !1) as *const u8, jit_size) };
            dump.load(&format!("ARM7_{guest_pc:x}"), code, thumb);
        }
    }
...
pub struct JitMemory {
    ...
    /// Per live-range page: invalidation-hit counters (mirror-collapsed), one flat
    /// array per code region. Pages hitting STICKY_SMC_THRESHOLD stay interpreted; see
    /// is_sticky_smc. Flat arrays: the check runs on every block dispatch.
    sticky_smc_ewram: HeapArray<u8, { (regions::EWRAM_SIZE >> MMU_PAGE_SHIFT) as usize }>,
    sticky_smc_iwram: HeapArray<u8, { (regions::IWRAM_SIZE >> MMU_PAGE_SHIFT) as usize }>,
    sticky_pages: u32,
}

/// Invalidation hits before a page is considered permanently self-modifying.
const STICKY_SMC_THRESHOLD: u8 = 4;

impl Emu {
    pub fn jit_protect_region(&mut self, guest_pc: u32, guest_pc_end: u32, thumb: bool, region: &MemRegion) {
        let guest_pc_end = guest_pc_end - if thumb { 2 } else { 4 };
        let begin = guest_pc >> MMU_PAGE_SHIFT;
        let end = guest_pc_end >> MMU_PAGE_SHIFT;
        for i in begin..=end {
            self.mmu_remove_write(i << MMU_PAGE_SHIFT, region);
        }
    }
    ...
    #[cfg(target_arch = "arm")]
    pub fn jit_insert_block(&mut self, block_asm: BlockAsm, debug_info: &JitDebugInfo, guest_pc: u32, guest_pc_end: u32, thumb: bool) -> (*const extern "C" fn(u32), bool) {
        // Byte-identity gate for backend refactors (the armv7-sacred check): with
        // ADVANCEDSLOP_BLOCK_HASH_LOG=<path> every compiled block's pre-relocation bytes are
        ...
        #[cfg(target_os = "linux")]
        block_hash_log(guest_pc, thumb, block_asm.get_code_buffer());
        macro_rules! insert {
            ($entries:expr, $region:expr) => {{
                let ret = insert!($entries);
                self.jit_protect_region(guest_pc, guest_pc_end, thumb, &$region);
                ret
            }};

            ($entries:expr) => {{
                let (allocated_offset_addr, aligned_size, flushed) = self.jit.insert(block_asm);
                ...
                self.jit.get_jit_data().jit_funcs.push_back(metadata);
                ...
                for &(pc, offset, size) in &debug_info.blocks {
                    self.jit.jit_perf_map_record.record(jit_entry_addr as usize + offset, size, pc, thumb);
                }
        ...
        match guest_pc & 0xFF000000 {
            // EWRAM is not write-protected: commercial games don't self-modify EWRAM
            // (only the m4a mixer in IWRAM does), so SMC detection there is pure overhead.
            // Compile it like ROM — track the block but leave its writes on the fast path.
            regions::EWRAM_OFFSET => insert!(self.jit.jit_entries.ewram),
            regions::IWRAM_OFFSET => insert!(self.jit.jit_entries.iwram, regions::IWRAM_REGION),
            regions::ROM_OFFSET..regions::ROM_END => insert!(self.jit.jit_entries.rom),
            _ => todo!("{:x}", guest_pc),
        }
    }
}
...
impl JitMemory {
    pub fn new() -> Self {
        ...
        JitMemory {
            ...
            sticky_smc_ewram: HeapArray::default(),
            sticky_smc_iwram: HeapArray::default(),
            sticky_pages: 0,
        }
    }
    ...
    fn get_jit_data(&mut self) -> &mut JitMemoryMetadata {
        &mut self.arm7_data
    }
    ...
    fn reset_blocks(&mut self) {
        ...
        let block_metadata = self.get_jit_data().jit_funcs.pop_front().unwrap();
        self.jit_memory_map
        ...
        let jit_size = self.get_jit_data().size;
        ...
        while (freed_end - freed_start) < (jit_size / 4 / PAGE_SIZE) as u16 {
            let block_metadata = self.get_jit_data().jit_funcs.front().unwrap();
            ...
            self.get_jit_data().jit_funcs.pop_front().unwrap();
        }

        let jit_data = self.get_jit_data();
        jit_data.start = (freed_start as usize) << PAGE_SHIFT;
        jit_data.end = (freed_end as usize) << PAGE_SHIFT;

        debug_println!("Jit memory reset from {:x} - {:x}", jit_data.start, jit_data.end);
    }
    ...
    #[cfg(target_arch = "arm")]
    fn insert(&mut self, block_asm: BlockAsm) -> (usize, usize, bool) {
    ...
    pub fn get_jit_start_addr(&self, guest_pc: u32) -> *const extern "C" fn(u32) {
        let entry = self.jit_memory_map.get_jit_entry(guest_pc);
        // Null map block + in-block offset = a small bogus pointer
        #[cfg(debug_assertions)]
        if (entry as usize) < 0x10000 {
            let regs = crate::core::thread_regs();
            eprintln!("dispatch to non-executable guest pc {guest_pc:x}");
            eprintln!("regs: {:x?} sp {:x} lr {:x} pc {:x} cpsr {:x}", regs.gp_regs, regs.sp, regs.lr, regs.pc, regs.cpsr);
            // Dump guest code/stack context through fastmem (reads only)
            let base = crate::core::MMU_TCM_ADDR;
            let dump = |name: &str, addr: u32, words: usize| {
                let mut line = format!("{name} {addr:x}:");
                for i in 0..words {
                    let word = unsafe { ((base + (addr as usize & 0x0FFFFFFF) + i * 4) as *const u32).read_unaligned() };
                    line += &format!(" {word:08x}");
                }
                eprintln!("{line}");
            };
            if regs.lr & 0x0F000000 == 0x03000000 {
                dump("code around lr", (regs.lr & !3).wrapping_sub(0x20), 12);
            }
            dump("stack", regs.sp.wrapping_sub(0x20), 16);
            // m4a bios sound-info pointer + struct head
            dump("iwram top", 0x3007FE0, 8);
            let sound_info = unsafe { ((base + 0x3007FF0usize) as *const u32).read_unaligned() };
            if sound_info & 0x0F000000 == 0x02000000 || sound_info & 0x0F000000 == 0x03000000 {
                dump("sound info", sound_info, 16);
            }
            panic!("dispatch to non-executable guest pc {guest_pc:x}");
        }
        unsafe { (*entry).0 }
    }

    #[inline(never)]
    /// Canonical page key for the sticky-SMC map: mirror-collapsed address of the
    /// live-range page the write landed in.
    fn sticky_counter(&mut self, guest_addr: u32) -> Option<&mut u8> {
        match guest_addr & 0x0F000000 {
            regions::EWRAM_OFFSET => Some(&mut self.sticky_smc_ewram[((guest_addr & (regions::EWRAM_SIZE - 1)) >> MMU_PAGE_SHIFT) as usize]),
            regions::IWRAM_OFFSET => Some(&mut self.sticky_smc_iwram[((guest_addr & (regions::IWRAM_SIZE - 1)) >> MMU_PAGE_SHIFT) as usize]),
            _ => None,
        }
    }

    /// Pages that keep getting written while holding compiled code (the m4a mixer
    /// patches its inner loops every pass) stop being worth compiling: after a few
    /// invalidation hits the page turns sticky and blocks touching it stay on the
    /// interpreter, which simply re-reads the patched bytes each pass.
    #[inline]
    pub fn is_sticky_smc(&self, guest_addr: u32) -> bool {
        if self.sticky_pages == 0 {
            return false;
        }
        let count = match guest_addr & 0x0F000000 {
            regions::EWRAM_OFFSET => self.sticky_smc_ewram[((guest_addr & (regions::EWRAM_SIZE - 1)) >> MMU_PAGE_SHIFT) as usize],
            regions::IWRAM_OFFSET => self.sticky_smc_iwram[((guest_addr & (regions::IWRAM_SIZE - 1)) >> MMU_PAGE_SHIFT) as usize],
            _ => return false,
        };
        count >= STICKY_SMC_THRESHOLD
    }

    pub fn any_sticky_smc(&self, start: u32, end: u32) -> bool {
        if self.sticky_pages == 0 {
            return false;
        }
        let mut addr = start & !(JIT_LIVE_RANGE_PAGE_SIZE - 1);
        while addr < end {
            if self.is_sticky_smc(addr) {
                return true;
            }
            addr += JIT_LIVE_RANGE_PAGE_SIZE;
        }
        false
    }

    /// Clear all compiled code + live-range bits in the 4 KB mmu page. Used when a page
    /// goes sticky: the whole page becomes interpret-only, so it can be un-protected.
    pub fn invalidate_mmu_page(&mut self, guest_addr: u32) {
        let page_base = guest_addr & !(MMU_PAGE_SIZE as u32 - 1);
        self.jit_memory_map.write_jit_entries(page_base, MMU_PAGE_SIZE, DEFAULT_JIT_ENTRY);
        let mut addr = page_base;
        while addr < page_base + MMU_PAGE_SIZE as u32 {
            let live_range = self.jit_memory_map.get_live_range(addr);
            if !live_range.is_null() {
                unsafe { *live_range = 0 };
            }
            let counts = self.jit_memory_map.get_exec_count(addr);
            if !counts.is_null() {
                unsafe { std::slice::from_raw_parts_mut(counts, (JIT_LIVE_RANGE_PAGE_SIZE >> 1) as usize).fill(0) };
            }
            addr += JIT_LIVE_RANGE_PAGE_SIZE;
        }
    }

    pub fn invalidate_block(&mut self, guest_addr: u32, size: usize) -> bool {
        let mut hit = false;
            ...
            ($guest_addr:expr) => {{
                ...
                if unlikely(*live_range & (1 << live_ranges_bit) != 0) {
                    ...
                    *live_range &= !(1 << live_ranges_bit);
                    ...
                    // Self-modifying code demotes itself to the interpreter: zero the
                    // hotness counts so the rewritten code re-earns compilation (the
                    // saturated counter would otherwise recompile on the next dispatch —
                    // the m4a mixer patches its own inner loop every call)
                    let counts = self.jit_memory_map.get_exec_count(guest_addr_start);
                    if !counts.is_null() {
                        unsafe { std::slice::from_raw_parts_mut(counts, (JIT_LIVE_RANGE_PAGE_SIZE >> 1) as usize).fill(0) };
                    }

                    if let Some(count) = self.sticky_counter($guest_addr) {
                        let new = count.saturating_add(1);
                        *count = new;
                        if new == STICKY_SMC_THRESHOLD {
                            self.sticky_pages += 1;
                        }
                    }
                    self.invalidate_overlapping_blocks($guest_addr);
                }
            }};
        }
    ...
    /// A compiled block whose entry lives in a neighboring live-range page can SPAN the
    /// written address: clearing only the page's entries leaves the block reachable
    /// through its own entry, still executing the pre-write code (the m4a frequency
    /// switch rewrites the multi-page mixer and the stale converter kept playing —
    /// half-written pcm chunks, audible as per-frame dropouts). Kill the entries of
    /// every block whose span contains the written address, over its whole range.
    fn invalidate_overlapping_blocks(&mut self, guest_addr: u32) {
        let mut spans: [(u32, u32); 8] = [(0, 0); 8];
        let mut count = 0;
        for block in &self.arm7_data.jit_funcs {
            let start = block.guest_pc & !1;
            let end = block.guest_pc_end & !1;
            if start <= guest_addr && guest_addr < end {
                if count < spans.len() {
        ...
        for &(start, end) in &spans[..count] {
            self.jit_memory_map.write_jit_entries(start, (end - start) as usize, DEFAULT_JIT_ENTRY);
            // Demote the whole span: without zeroing the hotness counts the killed block
            // recompiles on its very next dispatch and the per-pass self-patch thrashes
            // compile/invalidate every frame. Interpreted, the patched code is simply
            // re-read each pass and re-earns compilation like any cold block.
            for page in (start & !(JIT_LIVE_RANGE_PAGE_SIZE - 1)..end).step_by(JIT_LIVE_RANGE_PAGE_SIZE as usize) {
                let counts = self.jit_memory_map.get_exec_count(page);
                if !counts.is_null() {
                    unsafe { std::slice::from_raw_parts_mut(counts, (JIT_LIVE_RANGE_PAGE_SIZE >> 1) as usize).fill(0) };
                }
            }
    ...
    #[cfg(target_arch = "arm")]
    fn get_inst_mem_handler_fun(is_write: bool, transfer: SingleTransfer, guest_memory_addr: u32, cpsr_dirty: bool, io_func: &mut Option<*const ()>) -> *const () {
        macro_rules! _get_inst_mem_handler_fun {
            ($is_write:expr, $size:expr, $signed:expr, $write_func:ident, $read_func:ident, $read_func_64:ident) => {
                match ($is_write, $size) {
                    (true, 0) => $write_func::<{ MemoryAmount::Byte }> as _,
                    (true, 1) => $write_func::<{ MemoryAmount::Half }> as _,
                    (true, 2) => $write_func::<{ MemoryAmount::Word }> as _,
                    (true, 3) => $write_func::<{ MemoryAmount::Double }> as _,
                    (false, 0) => {
                        if $signed {
                            $read_func::<{ MemoryAmount::Byte }, true> as _
                        } else {
                            $read_func::<{ MemoryAmount::Byte }, false> as _
                        }
                    }
                    (false, 1) => {
                        if $signed {
                            $read_func::<{ MemoryAmount::Half }, true> as _
                        } else {
                            $read_func::<{ MemoryAmount::Half }, false> as _
                        }
                    }
                    (false, 2) => $read_func::<{ MemoryAmount::Word }, false> as _,
                    (false, 3) => $read_func_64 as _,
        ...
        if transfer.size() != 3 && guest_memory_addr & 0xFF000000 == regions::IO_PORTS_OFFSET {
            ...
            // DMA and sound writes must run the full slow path (breakout_imm)
            let dma_range = 0xB0..=0xDF;
            let sound_range = 0x60..=0xA8;
            let size = 1 << transfer.size();
            let aligned_addr = io_addr & !(size - 1);
            *io_func = if !dma_range.contains(&io_addr) && !sound_range.contains(&io_addr) {
                if is_write {
                    io_gba::get_write_with_size(aligned_addr, size as usize).map(|f| f as _)
                } else {
                    io_gba::get_read_with_size(aligned_addr, size as usize).map(|f| f as _)
                }
            } else {
                None
            };
    ...
    #[cfg(target_arch = "arm")]
    fn get_inst_mem_multiple_handler_fun(
        is_write: bool,
        ...
        macro_rules! _get_inst_mem_multiple_handler_fun {
            ($write_func:ident, $read_func:ident) => {
                match (is_write, transfer.write_back(), !transfer.add(), valid, user, needs_pc) {
                    (false, false, false, false, false, false) => $read_func::<false, false, false, false, false> as _,
                    (true, false, false, false, false, false) => $write_func::<false, false, false, false, false> as _,
                    (false, true, false, false, false, false) => $read_func::<true, false, false, false, false> as _,
                    (true, true, false, false, false, false) => $write_func::<true, false, false, false, false> as _,
                    (false, false, true, false, false, false) => $read_func::<false, true, false, false, false> as _,
                    (true, false, true, false, false, false) => $write_func::<false, true, false, false, false> as _,
                    (false, true, true, false, false, false) => $read_func::<true, true, false, false, false> as _,
                    (true, true, true, false, false, false) => $write_func::<true, true, false, false, false> as _,
                    (false, false, false, true, false, false) => $read_func::<false, false, true, false, false> as _,
                    (true, false, false, true, false, false) => $write_func::<false, false, true, false, false> as _,
                    (false, true, false, true, false, false) => $read_func::<true, false, true, false, false> as _,
                    (true, true, false, true, false, false) => $write_func::<true, false, true, false, false> as _,
                    (false, false, true, true, false, false) => $read_func::<false, true, true, false, false> as _,
                    (true, false, true, true, false, false) => $write_func::<false, true, true, false, false> as _,
                    (false, true, true, true, false, false) => $read_func::<true, true, true, false, false> as _,
                    (true, true, true, true, false, false) => $write_func::<true, true, true, false, false> as _,
                    (false, false, false, false, true, false) => $read_func::<false, false, false, true, false> as _,
                    (true, false, false, false, true, false) => $write_func::<false, false, false, true, false> as _,
                    (false, true, false, false, true, false) => $read_func::<true, false, false, true, false> as _,
                    (true, true, false, false, true, false) => $write_func::<true, false, false, true, false> as _,
                    (false, false, true, false, true, false) => $read_func::<false, true, false, true, false> as _,
                    (true, false, true, false, true, false) => $write_func::<false, true, false, true, false> as _,
                    (false, true, true, false, true, false) => $read_func::<true, true, false, true, false> as _,
                    (true, true, true, false, true, false) => $write_func::<true, true, false, true, false> as _,
                    (false, false, false, true, true, false) => $read_func::<false, false, true, true, false> as _,
                    (true, false, false, true, true, false) => $write_func::<false, false, true, true, false> as _,
                    (false, true, false, true, true, false) => $read_func::<true, false, true, true, false> as _,
                    (true, true, false, true, true, false) => $write_func::<true, false, true, true, false> as _,
                    (false, false, true, true, true, false) => $read_func::<false, true, true, true, false> as _,
                    (true, false, true, true, true, false) => $write_func::<false, true, true, true, false> as _,
                    (false, true, true, true, true, false) => $read_func::<true, true, true, true, false> as _,
                    (true, true, true, true, true, false) => $write_func::<true, true, true, true, false> as _,
                    (false, false, false, false, false, true) => $read_func::<false, false, false, false, true> as _,
                    (true, false, false, false, false, true) => $write_func::<false, false, false, false, true> as _,
                    (false, true, false, false, false, true) => $read_func::<true, false, false, false, true> as _,
                    (true, true, false, false, false, true) => $write_func::<true, false, false, false, true> as _,
                    (false, false, true, false, false, true) => $read_func::<false, true, false, false, true> as _,
                    (true, false, true, false, false, true) => $write_func::<false, true, false, false, true> as _,
                    (false, true, true, false, false, true) => $read_func::<true, true, false, false, true> as _,
                    (true, true, true, false, false, true) => $write_func::<true, true, false, false, true> as _,
                    (false, false, false, true, false, true) => $read_func::<false, false, true, false, true> as _,
                    (true, false, false, true, false, true) => $write_func::<false, false, true, false, true> as _,
                    (false, true, false, true, false, true) => $read_func::<true, false, true, false, true> as _,
                    (true, true, false, true, false, true) => $write_func::<true, false, true, false, true> as _,
                    (false, false, true, true, false, true) => $read_func::<false, true, true, false, true> as _,
                    (true, false, true, true, false, true) => $write_func::<false, true, true, false, true> as _,
                    (false, true, true, true, false, true) => $read_func::<true, true, true, false, true> as _,
                    (true, true, true, true, false, true) => $write_func::<true, true, true, false, true> as _,
                    (false, false, false, false, true, true) => $read_func::<false, false, false, true, true> as _,
                    (true, false, false, false, true, true) => $write_func::<false, false, false, true, true> as _,
                    (false, true, false, false, true, true) => $read_func::<true, false, false, true, true> as _,
                    (true, true, false, false, true, true) => $write_func::<true, false, false, true, true> as _,
                    (false, false, true, false, true, true) => $read_func::<false, true, false, true, true> as _,
                    (true, false, true, false, true, true) => $write_func::<false, true, false, true, true> as _,
                    (false, true, true, false, true, true) => $read_func::<true, true, false, true, true> as _,
                    (true, true, true, false, true, true) => $write_func::<true, true, false, true, true> as _,
                    (false, false, false, true, true, true) => $read_func::<false, false, true, true, true> as _,
                    (true, false, false, true, true, true) => $write_func::<false, false, true, true, true> as _,
                    (false, true, false, true, true, true) => $read_func::<true, false, true, true, true> as _,
                    (true, true, false, true, true, true) => $write_func::<true, false, true, true, true> as _,
                    (false, false, true, true, true, true) => $read_func::<false, true, true, true, true> as _,
                    (true, false, true, true, true, true) => $write_func::<false, true, true, true, true> as _,
                    (false, true, true, true, true, true) => $read_func::<true, true, true, true, true> as _,
                    (true, true, true, true, true, true) => $write_func::<true, true, true, true, true> as _,
                }
            };
        }
    ...
    #[cfg(target_arch = "arm")]
    unsafe fn execute_patch_slow_mem<const THUMB: bool>(host_pc: &mut usize, guest_memory_addr: u32, fast_mem: &mut [u8], guest_inst_metadata: &mut GuestInstMetadata) {
        ...
        if slow_mem_length == 0 {
            if guest_inst_metadata.s.fast.op.is_single_mem_transfer() {
                ...
                let inst_mem_func = Self::get_inst_mem_handler_fun(is_write, transfer, guest_memory_addr, guest_inst_metadata.dirty_guest_regs.is_reserved(Reg::CPSR), &mut io_func);
                ...
                let inst_mem_func = Self::get_inst_mem_multiple_handler_fun(
                        is_write,
                        transfer,
    ...
    #[cfg(target_arch = "arm")]
    pub unsafe fn patch_slow_mem(&mut self, host_pc: &mut usize, guest_memory_addr: u32, _: &ArmContext) -> bool {
        ...
        debug_println!("slow mem patch at {:x} {:?} addr {guest_memory_addr:x}", metadata.pc, metadata.s.fast.op);

        let thumb = metadata.pc & 1 == 1;
        if thumb {
            Self::execute_patch_slow_mem::<true>(host_pc, guest_memory_addr, fast_mem, metadata);
        } else {
            Self::execute_patch_slow_mem::<false>(host_pc, guest_memory_addr, fast_mem, metadata);
        }
        true
    }
}
