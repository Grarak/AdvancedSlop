...
// ARMv4 quirk: TST/TEQ/CMP/CMN with Rd=15 additionally restore CPSR from SPSR
// (mode/thumb switch, result discarded, no pipeline flush) — jsmolka arm.gba 234/235.
#[inline]
fn test_rd15_quirk(ctx: &mut Ctx, opcode: u32) {
    if unlikely(opcode & 0xF000 == 0xF000) {
        ctx.asm.emu.thread_restore_spsr();
    }
}
...
pub(super) fn mrs_rs(ctx: &mut Ctx, opcode: u32) -> InstResult {
    let value = *ctx.asm.emu.thread_get_reg(Reg::SPSR);
    ctx.set_reg((opcode >> 12) & 0xF, value);
    InstResult::Continue(1)
}
...
fn msr_apply(ctx: &mut Ctx, opcode: u32, value: u32, spsr: bool) -> InstResult {
    ...
    if spsr {
        ctx.asm.emu.thread_set_spsr_with_flags(value, flags);
    } else {
        ctx.asm.emu.thread_set_cpsr_with_flags(value, flags);
    }
    InstResult::Continue(1)
}
...
// ---------------------------------------------------------------------------------------------
// ARMv5 DSP extensions (absent on the ARM7TDMI): saturating add/sub and signed halfword multiplies.
...
macro_rules! qalu {
    ($name:ident, |$ctx:ident, $a:ident, $b:ident| $body:expr) => {
        pub(super) fn $name($ctx: &mut Ctx, opcode: u32) -> InstResult {
            // ARMv5 DSP op: no-op on the ARM7TDMI.
            let _ = &$ctx;
