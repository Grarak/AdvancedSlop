...
fn ldrh_strh<const LOAD: bool, const SIZE: u8, const SIGNED: bool, const ADDR: u8, const UP: bool, const REG: bool>(ctx: &mut Ctx, opcode: u32) -> InstResult {
    ...
    if LOAD {
        let value = if SIZE == 0 {
            ...
            (mem_read!(ctx.asm, u16, addr) as u32).rotate_right((addr & 1) << 3)
        };
        if ADDR != 0 {
            ctx.set_reg(rn, offset_addr);
        }
...
fn ldm_stm<const LOAD: bool, const PRE: bool, const UP: bool, const USER: bool, const WB: bool>(ctx: &mut Ctx, opcode: u32) -> InstResult {
    ...
    // ARM7 quirk: an empty register list transfers PC and moves the base by 0x40
    // (jsmolka arm.gba 513/514; commercial games never emit this shape).
    if unlikely(reg_list == 0) {
        let base = ctx.reg(rn);
        let addr = match (UP, PRE) {
            (true, false) => base,                    // IA
            (true, true) => base.wrapping_add(4),     // IB
            (false, true) => base.wrapping_sub(0x40), // DB
            (false, false) => base.wrapping_sub(0x3C), // DA
        };
        if WB {
            ctx.set_reg(rn, if UP { base.wrapping_add(0x40) } else { base.wrapping_sub(0x40) });
        }
        return if LOAD {
            let target = mem_read!(ctx.asm, u32, addr);
            InstResult::Branch(2, target)
        } else {
            let value = ctx.inst_addr + 12;
            mem_write!(ctx.asm, u32, addr, value);
    ...
    let user_banked = USER && (reg_list & (1 << 15) == 0) && !ctx.asm.emu.thread_is_user_mode();
    ...
    if LOAD {
        ...
        for i in 0..16u32 {
            ...
            if i == 15 {
                ...
                // get_reg_usr_mut.
                *ctx.asm.emu.thread_get_reg_usr_mut(crate::jit::reg::Reg::from(i as u8)) = value;
            } else {
                ctx.set_reg(i, value);
            }
        }

        // Base in the list: the loaded value wins on the ARM7TDMI — armwrestler's LDM
        // base-in-list cases, NooDS parity.
        if WB {
        ...
        if USER && branch_target.is_some() {
            ctx.asm.emu.thread_restore_spsr();
        }
        ...
        for i in 0..16u32 {
            ...
            let value = if i == 15 {
                ...
                *ctx.asm.emu.thread_get_reg_usr(crate::jit::reg::Reg::from(i as u8))
            } else if WB && i == rn && (reg_list & ((1 << rn) - 1)) != 0 {
                // ARM7TDMI STM with the base in the list stores the written-back base
                // unless it is the first listed register (NooDS parity).
                final_base
            } else {
                ctx.reg(i)
            };
...
// ---------------------------------------------------------------------------------------------
// LDRD/STRD (ARMv5, absent on the ARM7TDMI; the disassembler rejects odd/pc rd encodings as UnkArm) and SWP/SWPB.
...
fn ldrd_strd<const STORE: bool, const ADDR: u8, const UP: bool, const REG: bool>(ctx: &mut Ctx, opcode: u32) -> InstResult {
    // ARMv5E only: a no-op on the ARM7TDMI (like NooDS).
    let _ = (ctx, opcode);
