...
pub(super) fn ldrh_reg_t(ctx: &mut Ctx, opcode: u16) -> InstResult {
    let addr = reg_offset_addr(ctx, opcode);
    let value = (mem_read!(ctx.asm, u16, addr) as u32).rotate_right((addr & 1) << 3);
    ctx.set_reg((opcode & 0x7) as u32, value);
    InstResult::Continue(3)
}
...
pub(super) fn ldmia_t(ctx: &mut Ctx, opcode: u16) -> InstResult {
    ...
    // ARM7 quirk: empty rlist loads PC and moves the base by 0x40
    if count == 0 {
        let target = mem_read!(ctx.asm, u32, addr);
        ctx.set_reg(rb, addr.wrapping_add(0x40));
        return InstResult::Branch(2, target | 1);
    }
...
pub(super) fn stmia_t(ctx: &mut Ctx, opcode: u16) -> InstResult {
    ...
    // ARM7 quirk: empty rlist stores PC+6 and moves the base by 0x40
    if count == 0 {
        let value = ctx.inst_addr + 6;
        mem_write!(ctx.asm, u32, addr, value);
        ctx.set_reg(rb, addr.wrapping_add(0x40));
        return InstResult::ContinueStore(1);
    }
...
pub(super) fn pop_pc_t(ctx: &mut Ctx, opcode: u16) -> InstResult {
    ...
    ctx.set_reg(13, sp.wrapping_add(count as u32 * 4));
    // ARMv4 stays in thumb on a pc pop (ARMv5 would interwork on bit 0).
    let target = (value & !1) | 1;
