...
use crate::jit::inst_thread_regs_handler::{register_restore_spsr_checked, register_set_cpsr_checked, register_set_spsr_checked};
...
impl JitAsm<'_> {
    pub fn emit_msr(&mut self, inst_index: usize, block_asm: &mut BlockAsm) {
        ...
        let (func, save_regs) = match inst.op {
            Op::MsrRc | Op::MsrIc => (register_set_cpsr_checked as *const (), flags & 1 == 1),
            Op::MsrRs | Op::MsrIs => (register_set_spsr_checked as *const (), false),
            _ => unreachable!(),
        };
    ...
    // ARMv4 quirk: TST/TEQ/CMP/CMN with Rd=15 = CPSR := SPSR (result discarded, no
    // flush). Same call shape as emit_msr's mode-changing path: save dirty regs, swap
    // banks in the handler, resync host flags, reload the banked window.
    pub fn emit_test_rd15(&mut self, inst_index: usize, block_asm: &mut BlockAsm) {
        let inst = &self.jit_buf.insts[inst_index];
        block_asm.save_dirty_guest_regs(true, inst.cond == Cond::AL);
        block_asm.call(register_restore_spsr_checked as *const ());
        block_asm.msr2(MaskedSpecialRegisterType_CPSR_f.into(), &Reg::R0.into());
        block_asm.restore_guest_regs_ptr();
        const REG_TO_RESTORE: RegReserve = reg_reserve!(Reg::R8, Reg::R9, Reg::R10, Reg::R11, Reg::R12, Reg::SP, Reg::LR);
        block_asm.reload_active_guest_regs(REG_TO_RESTORE);
