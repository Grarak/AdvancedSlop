...
impl JitAsm<'_> {
    ...
    pub fn emit_clz(&mut self, _inst_index: usize, _block_asm: &mut BlockAsm) {
        // ARMv5 encoding the ARM7TDMI doesn't have. The decode tables 
        // still map it here, but a real game only reaches it after derailing into data.
    }

    pub fn emit_q_op(&mut self, _inst_index: usize, _block_asm: &mut BlockAsm) {
        // ARMv5 encoding the ARM7TDMI doesn't have. The decode tables 
        // still map it here, but a real game only reaches it after derailing into data.
    }
}
