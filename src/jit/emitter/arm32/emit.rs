...
impl JitAsm<'_> {
    ...
    fn handle_indirect_branch(&mut self, inst_index: usize, block_asm: &mut BlockAsm) {
        ...
        if restore_spsr {
            block_asm.call(register_restore_spsr as *const ());
        }

        if restore_spsr {
            block_asm.call(restore_thumb_after_restore_spsr as *const ());
        } else {
            block_asm.call(set_pc_arm_mode as *const ());
        }
            ...
            || (inst.op.is_single_mem_transfer() && inst.src_regs.is_reserved(Reg::SP))
        {
            match inst.op {
                Op::Ldm(transfer) | Op::Stm(transfer) => {
                    if transfer.user() {
                        block_asm.call(register_restore_spsr as *const ());
                        block_asm.call(set_pc_arm_mode as _);
                    }
                }
                _ => {}
            }
    ...
    fn handle_indirect_branch_thumb(&mut self, inst_index: usize, block_asm: &mut BlockAsm) {
        ...
        if !inst.op.is_multiple_mem_transfer() {
            block_asm.call(set_pc_thumb_mode as *const ());
        }
    ...
    fn emit_basic_block(&mut self, basic_block_index: usize, block_asm: &mut BlockAsm, thumb: bool) {
        ...
        debug_println!("emit basic block {basic_block_index}");
        ...
        for i in start_index..end_index + 1 {
            ...
            let inst = &self.jit_buf.insts[i];
            if i != 0 {
                block_asm.guest_offset(self.jit_buf.insts_cycle_counts[i] - inst.cycle as u16, self.emu);
            }
            ...
            if thumb {
                ...
                match inst.op {
                    Op::Mcr | Op::Mrc => {} // No coprocessor on the GBA's ARM7TDMI
                    Op::Tst | Op::Teq | Op::Cmp | Op::Cmn if inst.opcode & 0xF000 == 0xF000 => self.emit_test_rd15(i, block_asm),
            ...
            if crate::INST_TRACE {
                ...
                block_asm.call(debug_after_exec_op as *const ());
