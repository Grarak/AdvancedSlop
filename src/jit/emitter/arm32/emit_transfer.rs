...
impl JitAsm<'_> {
    ...
    fn emit_fast_single_write_transfer<F: FnOnce(&Self, &mut BlockAsm)>(&mut self, flag_update: u32, op0: Reg, op0_next: Reg, tmp_reg: Reg, size: u8, metadata_emitter: F, block_asm: &mut BlockAsm) {
        ...
        block_asm.mov4(flag_update, Cond::AL, Reg::LR, &(crate::core::MMU_TCM_ADDR as u32).into());
    ...
    fn emit_fast_single_read_transfer<F: FnOnce(&Self, &mut BlockAsm)>(
        ...
        block_asm.mov4(flag_update, Cond::AL, Reg::R0, &(crate::core::MMU_TCM_ADDR as u32).into());
        ...
        // NOTE: the misaligned-LDRH rotate quirk is handled by the interpreter and the
        // slow path only. Emitting the rotate here grew the fast window past the
        // slow-patch budget: after a site is patched, the leftover tail executed on top
        // of the slow handler's result (double rotate / clobber).
        if is_64bit {
            block_asm.add5(flag_update, Cond::AL, Reg::R2, Reg::R2, &4.into());
            Self::emit_align_addr(flag_update, tmp_reg, Reg::R2, 4, block_asm);
        ...
        let fast_mem_start = match inst.imm_transfer_addr(block_asm.current_pc) {
            Some(imm_addr) => {
                ...
                if consider_slow_mem {
                    ...
                    block_asm.ensure_emit_for(64);
                } else if !is_write && size == 4 && (0x08000000..0x0E000000).contains(&imm_addr) && self.analyzer.can_imm_load(imm_addr) {
                    // Fold only ROM literals: a RAM literal pool can be repatched by SMC
                    // (m4a's frequency switch rewrites the mixer's pc-relative loop count
                    // at a NEGATIVE offset, outside the block span — no invalidation ever
                    // fires for it, and the baked constant goes stale).
                    let imm_value = self.emu.mem_read::<u32>(imm_addr);
                    block_asm.ldr2(value_reg, imm_value);
                    return;
                }

                // ARM7 quirk: misaligned LDRSH loads a sign-extended byte (known at emit time)
                let misaligned_signed_half = !is_write && size == 2 && transfer.signed() && imm_addr & 1 != 0;
                let func = if is_write {
                    get_write_func!(size)
                } else if misaligned_signed_half {
                    get_read_func!(1, true)
                } else {
                    get_read_func!(size, transfer.signed())
                };

                let fast_mem_start = block_asm.get_cursor_offset();

                let aligned_addr = (imm_addr & !(0xF0000000 | (size as u32 - 1))) + misaligned_signed_half as u32;
                // Don't query write offset here, we don't want to invalidate the jit block
                let shm_offset = self.emu.get_shm_offset::<true, false>(aligned_addr);
                let host_addr = if is_write && shm_offset != 0 {
                    (self.emu.mem.shm.as_mut_ptr() as usize + shm_offset) as u32
                } else {
                    aligned_addr + crate::core::MMU_TCM_ADDR as u32
                };
                ...
                if !is_write {
                    ...
                    } else if size == 2 && !transfer.signed() && imm_addr & 1 != 0 {
                        // ARM7 quirk: misaligned LDRH rotates the aligned halfword
                        block_asm.ror5(flag_update, Cond::AL, value_reg, value_reg, &8.into());
                    }
                }
                if is_64bit {
    ...
    fn emit_multiple_transfer_fast(&mut self, inst_index: usize, basic_block_index: usize, transfer: MultipleTransfer, block_asm: &mut BlockAsm) {
        ...
        block_asm.add5(flag_update, Cond::AL, Reg::R0, Reg::R0, &(crate::core::MMU_TCM_ADDR as u32).into());
        ...
        if write_back {
            block_asm.sub5(flag_update, Cond::AL, op0_mapped, Reg::R0, &(crate::core::MMU_TCM_ADDR as u32).into());
        }
    ...
    pub fn emit_multiple_transfer(&mut self, inst_index: usize, basic_block_index: usize, block_asm: &mut BlockAsm) {
        ...
        if op1.is_empty() {
            // Usually data decoded as code (blocks span dead bytes). The base adjust is
            // emitted so the accounting stays sane; the full ARM7 pc-transfer quirk
            // lives in the interpreter — no game executes this shape for real.
            let op0_mapped = block_asm.get_guest_map(op0);
            let next_live_regs = self.analyzer.get_next_live_regs(basic_block_index, inst_index);
            let save_cpsr = block_asm.dirty_guest_regs.is_reserved(Reg::CPSR) || next_live_regs.is_reserved(Reg::CPSR);
            let flag_update = if save_cpsr { FlagsUpdate_LeaveFlags } else { FlagsUpdate_DontCare };
