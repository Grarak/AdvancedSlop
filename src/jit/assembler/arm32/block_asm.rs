...
impl BlockAsm {
    pub fn new(thumb: bool, is_os_irq_handler: bool) -> Self {
        BlockAsm {
            ...
            last_pc_value: 0,
            guest_regs_ptr: crate::core::GUEST_REGS_ADDR as _,
            is_os_irq_handler,
    ...
    pub fn guest_offset(&mut self, pre_cycle_count_sum: u16, emu: &Emu) {
        ...
        for (i, guest_reg) in self.reg_alloc.host_regs_mapping.iter().enumerate() {
            match guest_reg {
                ...
                _ => inst_offset.mapping[i] = emu.thread_get_reg(*guest_reg) as _,
            }
        }
    }
