...
// re-exports the real disassembler island). `cpu` is a raw u8 (always 1 on the GBA; the
// fixed record layout keeps the byte so the offline trace tools stay compatible).
...
#[inline]
pub fn cpu_name(_cpu: u8) -> &'static str {
    "ARM7"
}
    ...
    while reader.read_exact(&mut tag).is_ok() {
        match tag[0] {
            ...
            TAG_INST_DELTA => {
                ...
                let cpu_index = 0usize; // single-cpu logs: keyframes stamp cpu=0
                let _ = FLAG_CPU7;
