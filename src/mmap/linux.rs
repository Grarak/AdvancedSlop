...
impl Shm {
    pub fn new(name: impl AsRef<str>, size: usize) -> io::Result<Self> {
        let name = format!("advancedslop_{}", name.as_ref());
        let name = CString::new(name)?;
        // memfd instead of shm_open: same anonymous-fd semantics for the MAP_SHARED
        // mirror maps, without a /dev/shm name to create and clean up.
...
unsafe extern "C" fn sigsegv_handler(sig: i32, si: *mut siginfo_t, segfault_ctx: *mut c_void) {
    ...
    // armv7 mcontext: contiguous arm_r0..arm_lr match ArmContext's layout.
    #[cfg(target_arch = "arm")]
