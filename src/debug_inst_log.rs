...
struct PrevStates(UnsafeCell<[PrevState; 1]>);
...
// Optional record budget (ADVANCEDSLOP_INST_LOG_MAX): after this many instruction records the log
...
#[cfg(target_os = "linux")]
fn init_budget() {
    let max = std::env::var("ADVANCEDSLOP_INST_LOG_MAX").ok().and_then(|v| v.parse::<u64>().ok()).unwrap_or(0);
    unsafe { *RECORD_BUDGET.0.get() = max };
    // ADVANCEDSLOP_INST_LOG_TEXT=0 drops the interleaved text records (debug_println/branch_println
    ...
    let text = std::env::var("ADVANCEDSLOP_INST_LOG_TEXT").map(|v| v != "0").unwrap_or(true);
...
#[inline]
pub fn log(emu: &Emu, pc: u32, opcode: u32) {
    // Gated capture: ADVANCEDSLOP_LOG_FROM_CYCLE skips records until the guest clock reaches
    // the threshold; ADVANCEDSLOP_LOG_FROM_VBLANK skips until the Nth delivered vblank irq
    // (guest-synchronous — cycle gates land at engine-dependent in-frame positions and
    // misalign A/B traces).
    unsafe {
        static mut FROM: u64 = u64::MAX;
        if FROM == u64::MAX {
            FROM = std::env::var("ADVANCEDSLOP_LOG_FROM_CYCLE").ok().and_then(|v| v.parse().ok()).unwrap_or(0);
        }
        if (emu.cm.get_cycles() as u64) < FROM {
            return;
        }
        if !VBLANK_GATE_OPEN() {
            return;
    ...
    if *budget != 0 {
        *budget -= 1;
        if *budget == 0 {
            ...
            write_record(writer, emu, pc, opcode);
            let _ = writer.flush();
            ...
            // exactly that class.
            {
                ...
                let main = unsafe {
                    std::slice::from_raw_parts(
                        emu.mem.shm.as_ptr().add(crate::core::memory::regions::EWRAM_REGION.shm_offset),
                        crate::core::memory::regions::EWRAM_REGION.size,
                    )
                };
                let vram = unsafe { std::slice::from_raw_parts(emu.mem.shm.as_ptr().add(crate::core::memory::regions::VRAM_REGION.shm_offset), crate::core::memory::regions::VRAM_SIZE as usize) };
                eprintln!("MEMHASH ewram={:08x} vram={:08x} palettes={:08x} oam={:08x}", xxh32(main, 0), xxh32(vram, 0), xxh32(palettes, 0), xxh32(oam, 0));
            }
            return;
        }
    }
...
#[inline]
fn write_record(writer: &mut BufWriter<File>, emu: &Emu, pc: u32, opcode: u32) {
    let mut regs = [0u32; 15];
    for (i, r) in regs.iter_mut().enumerate() {
        *r = *emu.thread_get_reg(Reg::from(i as u8));
    }
    let cpsr = *emu.thread_get_reg(Reg::CPSR);
    let spsr = *emu.thread_get_reg(Reg::SPSR);

    static PREV_STATES: PrevStates = PrevStates(UnsafeCell::new([PrevState::default_const()]));
    let prev = unsafe { &mut (*PREV_STATES.0.get())[0] };
...
#[inline]
pub fn log_mem(kind: MemLogKind, addr: u32, value: u32) {
    if !crate::DEBUG_LOG {
        return;
    }
    if !is_logging() {
        let current_thread = std::thread::current();
        println!("[{}] {}", current_thread.name().unwrap(), format_mem_line(0, kind as u8, addr, value));
        return;
    }
    if !TEXT_ENABLED.load(std::sync::atomic::Ordering::Relaxed) {
        return;
    ...
    let mut buf = [0u8; 10];
    buf[0] = TAG_MEM;
    buf[1] = mem_meta(0, kind, width_code);
...
#[inline]
pub fn log_mem_slice<T: Convert>(kind: MemLogKind, addr: u32, values: &[T]) {
    if !crate::DEBUG_LOG {
        return;
    }
    if !is_logging() {
        ...
        for (i, &value) in values.iter().enumerate() {
            println!("[{name}] {}", format_mem_line(0, kind as u8, addr.wrapping_add((i * stride) as u32), value.into()));
        }
        return;
    }
    ...
    let mut head = [0u8; 10];
    head[0] = TAG_MEM;
    head[1] = mem_meta(0, kind, width_code);
...
pub static mut VBLANK_GATE: (u32, u32) = (u32::MAX, 0); // (threshold, seen)

#[allow(non_snake_case)]
fn VBLANK_GATE_OPEN() -> bool {
    unsafe {
        let (threshold, seen) = *(&raw const VBLANK_GATE);
        if threshold == u32::MAX {
            let t = std::env::var("ADVANCEDSLOP_LOG_FROM_VBLANK").ok().and_then(|v| v.parse().ok()).unwrap_or(0);
            (*(&raw mut VBLANK_GATE)).0 = t;
            return t == 0;
        }
        seen >= threshold
    }
...
pub fn count_vblank_delivery() {
    unsafe { (*(&raw mut VBLANK_GATE)).1 += 1 };
}
